        ...
        window.addEventListener('load',function(){
            ...
            pagin.addEventListener('click', handlePagination);
        });
        ...
        function createPagination(){
            ...
            prev.name = curretPage - 1;
            if(curretPage <= 1){
                prev.disabled = true;
            }

            var current = document.createElement('button');
            current.name = curretPage;
            current.textContent = curretPage;

            var next = document.createElement('button');
            next.name = curretPage + 1;
        ...
        function handlePagination(e){
            const pageNum = parseInt(e.target.name);
            curretPage = pageNum;
            fetchUserData(curretPage, limit);
            createPagination();
        }

        function search(){
            ele = document.getElementById('input').value;
            console.log(ele)
            fetchUserData(curretPage, limit);
        }

        function fetchUserData(curretPage, limit){
            console.log("I'm")
            fetch(`https://newsapi.org/v2/everything?q=${ele}&page=${curretPage}&pageSize=${limit}&apiKey=${api_key}`)
            .then(function(response){
                return response.json();
            })
            .then(function(response){
        ...
        function postNewsOfSearch(newsData){
            console.log("I'm============")
            var page = document.getElementById('page');
            page.textContent = "";
